...
/**
 * This class tests out how the students in a course did on the assignment by
 * playing all pairs of students in games against one another. The names of the
 * possible players should be stored in the enum PlayerClassNames which can be
 * found in this same package (ttr.test)
 * */
public class FullCourseTester implements IEventListener {
	...
	int[][] p1Scores = new int[PlayerClassName.values().length][PlayerClassName
			.values().length];
	int[][] p2Scores = new int[PlayerClassName.values().length][PlayerClassName
			.values().length];
	int p1Index = 0;
	int p2Index = 0;
	int gameNum = 0;
	...
	/**
	 * Runs a single game and returns an array containing the score (player 1
	 * first, then player 2
	 * */
	private void playGame(PlayerClassName player1Name,
			PlayerClassName player2Name) {
		...
		TTRGamePlayScene scene = new TTRGamePlayScene("Ticket To Ride",
				"woodBacking.jpg", game, player1, player2);
	...
	/**
	 * given a student's class name, instantiates and returns an instance of
	 * that player
	 * */
	private Player getStudentPlayerObject(PlayerClassName className) {

		try {
			Class<?> playerClass = Class.forName("ttr.model.player."
					+ className);
			Constructor<?> constructor = playerClass
	...
	public void handleEvent(Event e) {
		...
		if (e.getEventType().equals(GameOverEvent.GAME_COMPLETED)) {
			...
			evt.getScene().removeEventListener(this,
					GameOverEvent.GAME_COMPLETED);
			int[] scores = { evt.getScene().getPlayer1Score(),
					evt.getScene().getPlayer2Score() };
	...
	private void printResults() {
		...
		System.out.println("p1: "+PlayerClassName.values()[0]+", "+p1+"\np2: "+p2);
